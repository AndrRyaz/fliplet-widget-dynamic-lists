...
DynamicList.prototype.goToAgendaFeature = function(options) {
  ...
  return Fliplet.App.Storage.set(`${type}SessionTitle-${screen.id}`, data).then(
    function() {
      return Fliplet.Navigate.screen(screen.id, {
        transition: 'fade',
        query: `?sessionId=${encodeURIComponent(
          entryUniqueId
        )}&sessionTitle=${encodeURIComponent(entryTitle)}`
      });
    }
  );
};
...
DynamicList.prototype.attachObservers = function() {
  ...
  _this.$container
    ...
    .on(
      'click keydown',
      '.agenda-detail-overlay-close, .agenda-detail-overlay-screen',
      function(event) {
        if (_this.Utils.Accessibility.accessibilityDetails(event, $(this))) {
          var result;
      ...
      if (!$(this).hasClass('placeholder')) {
        if (_this.Utils.Accessibility.accessibilityDetails(event, $(this))) {
          ...
          Fliplet.Analytics.trackEvent({
            category: `list_dynamic_${_this.data.layout}`,
            action: 'filter_date',
            label: `${$(this).find('.week').text().trim()} ${$(this)
              .find('.day')
              .text()
              .trim()} ${$(this).find('.month').text().trim()}`
          });
    ...
    .on(
      'click keydown',
      '.agenda-detail-overlay .bookmark-wrapper, .search-results-wrapper .bookmark-wrapper',
      function(event) {
        if (_this.Utils.Accessibility.accessibilityDetails(event, $(this))) {
...
DynamicList.prototype.parseFilterQueries = function() {
  ...
  if (!_.get(_this.pvFilterQuery, 'hideControls', false)) {
    ...
    _this.calculateFiltersHeight(
      _this.$container.find('.new-agenda-list-container')
    );
  }
};
...
DynamicList.prototype.groupLoopDataByDate = function(loopData, dateField) {
  ...
  _.forEach(recordDates, function(key, i) {
    ...
    _.forEach(recordDates, function(comp, j) {
      ...
      if (
        ...
        _this.Utils.Date.moment(comp).format('YYYY-MM-DD')
      ) {
        return;
      }
  ...
  _.forEach(recordMerges, function(merge) {
    recordGroups[merge.to] = _.concat(
      recordGroups[merge.to],
      recordGroups[merge.from]
    );
...
DynamicList.prototype.emptyLoop = function() {
  if (this.isInLoopView()) {
    ...
    this.$container
      .find('.search-results-wrapper .search-results-holder')
      .empty();
  }
};
...
DynamicList.prototype.renderLoopHTML = function() {
  ...
  if (this.isInLoopView()) {
    ...
    template =
      _this.data.advancedSettings &&
      _this.data.advancedSettings.searchResultsHTML
        ? Handlebars.compile(_this.data.advancedSettings.searchResultsHTML)
        : Handlebars.compile(
          Fliplet.Widget.Templates[
            _this.layoutMapping[_this.data.layout]['search-results']
          ]()
        );
  }
...
DynamicList.prototype.renderDatesHTML = function(records, index) {
  ...
  if (!dateField || _this.dataSourceColumns.indexOf(dateField) === -1) {
    throw new Error(
      'Date field is misconfigured. Please check your component settings.'
    );
  }
  ...
  clonedRecords = _.orderBy(
    _.filter(clonedRecords, function(record) {
  ...
  if (clonedRecords.length) {
    ...
    lastDate = _this.Utils.Date.moment(
      clonedRecords[clonedRecords.length - 1].data[dateField]
    );
    ...
    var uniqueDates = _.map(
      _.uniqBy(clonedRecords, function(obj) {
        return _this.Utils.Date.moment(obj.data[dateField]).format(
          'YYYY-MM-DD'
        );
      }),
      `data.${dateField}`
  ...
  var template =
    this.data.advancedSettings && this.data.advancedSettings.otherLoopHTML
      ? Handlebars.compile(this.data.advancedSettings.otherLoopHTML)
      : Handlebars.compile(
        Fliplet.Widget.Templates[
          this.layoutMapping[this.data.layout]['other-loop']
        ]()
      );

  this.$container
    .find('.agenda-date-selector ul')
    .html(template(calendarDates));
    ...
    return Promise.all(
      _.map(_.flatten(records), function(record) {
        var title = _this.$container
          .find(
            `.agenda-cards-wrapper .agenda-list-item[data-entry-id="${record.id}"] .agenda-item-title`
          )
          .text()
          .trim();
...
DynamicList.prototype.animateDateForward = function(
  nextDateElement,
  nextDateElementWidth
) {
  var _this = this;
...
DynamicList.prototype.animateAgendaForward = function(
  nextAgendaElement,
  nextAgendaElementWidth
) {
  var _this = this;
  ...
  return new Promise(function(resolve) {
    _this.$container.find('.agenda-cards-wrapper').animate(
      ...
      function() {
        _this.$container
          .find('.agenda-list-day-holder.active')
          .removeClass('active');
...
// animates dates back
DynamicList.prototype.animateDateBack = function(
  prevDateElement,
  prevDateElementWidth
) {
  var _this = this;
...
// animate cards back
DynamicList.prototype.animateAgendaBack = function(
  prevAgendaElement,
  prevAgendaElementWidth
) {
  var _this = this;
...
DynamicList.prototype.moveForwardDate = function(index, difference) {
  ...
  var nextDateElement = _this.$container
    .find('.agenda-date-selector li')
    .not('.placeholder')
    .eq(index);
  var nextAgendaElement = _this.$container
    .find('.agenda-list-day-holder')
    .eq(index);

  if (
    !nextDateElement.length ||
    !nextAgendaElement.length ||
    _this.animatingForward
  ) {
    return;
  }

  _this.animatingForward = true;
  var nextDateElementWidth = Math.floor(
    nextDateElement.outerWidth() * difference
  );
  var nextAgendaElementWidth = Math.floor(
    nextAgendaElement.outerWidth() * difference
  );
  ...
  Promise.all([
    ...
    _this.animateAgendaForward(
      nextAgendaElement,
      nextAgendaElementWidth - _this.scrollValue
    )
  ]).then(function() {
...
DynamicList.prototype.moveBackDate = function(index, difference) {
  ...
  var prevDateElement = _this.$container
    .find('.agenda-date-selector li')
    .not('.placeholder')
    .eq(index);
  var prevAgendaElement = _this.$container
    .find('.agenda-list-day-holder')
    .eq(index);
  var positiveDifference = (difference *= -1);

  if (
    !prevDateElement.length ||
    !prevAgendaElement.length ||
    _this.animatingBack
  ) {
    return;
  }
  ...
  var prevDateElementWidth = Math.floor(
    prevDateElement.outerWidth() * positiveDifference
  );
  var prevAgendaElementWidth = Math.floor(
    prevAgendaElement.outerWidth() * positiveDifference
  );
  ...
  Promise.all([
    ...
    _this.animateAgendaBack(
      prevAgendaElement,
      prevAgendaElementWidth + _this.scrollValue
    )
  ]).then(function() {
    _this.isPanning = false;
...
DynamicList.prototype.centerDate = function() {
  ...
  var activePosition = _this.$container
    .find('.agenda-date-selector li.active')
    .position();
  var activeWidth = _this.$container
    .find('.agenda-date-selector li.active')
    .outerWidth();
  ...
  _this.$container
    .find('.agenda-date-selector ul')
    .scrollLeft(activePosition.left - (halfWindowWidth - halfWidth));
};
...
DynamicList.prototype.toggleBookmarkStatus = function(record) {
  ...
  if (record.bookmarked) {
    this.$container
      ...
      .find(
        `.agenda-item-bookmark-holder-${record.id} .bookmark-wrapper.btn-bookmark`
      )
      .removeClass('btn-bookmark')
      .addClass('btn-bookmarked')
      .find('.fa-bookmark-o')
    ...
    this.$container
      .find('.agenda-cards-wrapper, .search-results-wrapper')
      .find(
        `.agenda-item-bookmark-holder-${record.id} .bookmark-wrapper.btn-bookmarked`
      )
      .removeClass('btn-bookmarked')
      .addClass('btn-bookmark')
      .find('.fa-bookmark')
  ...
  return _this.getBookmarkIdentifier(record).then(function(identifier) {
    return new Promise(function(resolve) {
      var btn = LikeButton({
        ...
        likeWrapper:
          '<div class="bookmark-wrapper focus-outline btn-bookmark" tabindex="0"></div>',
        likedWrapper:
          '<div class="bookmark-wrapper focus-outline btn-bookmarked" tabindex="0"></div>',
        addType: 'prepend',
        getAllCounts: false,
      ...
      btn.on('liked.fail', function() {
        record.bookmarked = btn.isLiked();
        _this.$container
          .find(`.agenda-detail-overlay .agenda-item-bookmark-holder-${id}`)
          .removeClass('bookmarked')
          .addClass('not-bookmarked');
        _this.$container
          .find(
            `.search-results-wrapper .agenda-item-bookmark-holder-${id} .bookmark-wrapper`
          )
          .removeClass('btn-bookmarked')
          .addClass('btn-bookmark');
...
DynamicList.prototype.initializeOverlaySocials = function(id) {
  ...
  var title = _this.$container
    .find(
      `.agenda-detail-overlay .agenda-detail-wrapper[data-entry-id="${id}"] .agenda-item-title`
    )
    .text()
    .trim();
...
DynamicList.prototype.isSubsetResult = function(data) {
  return (
    data.length &&
    data.length === _.intersectionBy(data, this.getListItems(), 'id').length
  );
};
...
DynamicList.prototype.isInLoopView = function() {
  return (
    ...
    !this.isSearching &&
    !this.showBookmarks &&
    !this.$container.find('.list-search-icon .fa-sliders.active').length
  );
};
...
DynamicList.prototype.removeFilteredEntries = function(data) {
  if (this.isInLoopView()) {
    ...
    var recordIdsToHide = _.difference(
      _.map(this.filteredListItemsByDay, 'id'),
      recordIdsToShow
    );

    // Hide and show content based on existing render
    this.$container
      .find('.agenda-cards-wrapper')
      .find(
    ...
    this.$container
      .find('.search-results-list-day-holder')
      .find(
        _.map(_.differenceBy(this.filteredListItems, data, 'id'), function(
          record
        ) {
...
DynamicList.prototype.searchData = function(options) {
  ...
  _this.showBookmarks = !!_this.$container.find(
    '.toggle-agenda.mixitup-control-active, .toggle-bookmarks.mixitup-control-active'
  ).length;
  ...
  return _this.Utils.Records.runSearch({
    ...
    return Fliplet.Hooks.run('flListDataBeforeRenderList', {
      ...
      .then(function() {
        ...
        _this.$container
          .find('.hidden-search-controls')
          [value.length ? 'addClass' : 'removeClass']('has-query');
        ...
        _this.setAgendasByDay(
          _this.groupLoopDataByDate(
            _this.modifiedListItems,
            _this.dateFieldLocation
          )
        );
        ...
        return _this.renderLoopHTML().then(function(records) {
          ...
          if (_this.isInLoopView()) {
            if (options.goToToday) {
              ...
              _this.sliderGoTo(
                $('.agenda-date-selector li.active').index() -
                  $('.agenda-date-selector li:not(.placeholder)').index()
              );
            }
          }
...
DynamicList.prototype.bindTouchEvents = function() {
  ...
  _this.hammer.on('panright panleft', function(e) {
    ...
    _this.activeSlideIndex = _this.$container
      .find('.agenda-list-day-holder')
      .index(_this.$container.find('.agenda-list-day-holder.active'));
    _this.$container
      .find('.agenda-date-selector, .agenda-date-selector ul')
      .addClass('is-panning');
    _this.scrollValue = -1 * e.deltaX;
    _this.$container
      .find('.agenda-cards-wrapper')
      .scrollLeft(_this.copyOfScrollValue + _this.scrollValue);
  });
...
DynamicList.prototype.addDetailViewData = function(entry) {
  ...
  var notDynamicData = _.filter(_this.data.detailViewOptions, function(
    option
  ) {
  ...
  dynamicData.forEach(function(dynamicDataObj) {
    ...
    // Define label
    if (
      dynamicDataObj.fieldLabel === 'column-name' &&
      dynamicDataObj.column !== 'custom'
    ) {
      label = dynamicDataObj.column;
    }
    if (dynamicDataObj.fieldLabel === 'custom-label') {
      label = new Handlebars.SafeString(
        Handlebars.compile(dynamicDataObj.customFieldLabel)(entry.originalData)
      );
    }
    ...
    if (dynamicDataObj.customFieldEnabled) {
      content = new Handlebars.SafeString(
        Handlebars.compile(dynamicDataObj.customField)(entry.originalData)
      );
    } else {
...
DynamicList.prototype.showDetails = function(id, listData) {
  ...
  var _this = this;
  var entryData =
    _.find(listData, { id: id }) ||
    _(_this.getAgendasByDay())
      .chain()
      .thru(function(coll) {
  ...
  var entryId = { id: id };
  var wrapper =
    '<div class="agenda-detail-wrapper" data-entry-id="{{id}}"></div>';
  ...
  return beforeShowDetails.then(function(data) {
    ...
    $overlay
      .find('.agenda-detail-overlay-content-holder')
      .html(wrapperTemplate(entryId));
    $overlay
      .find('.agenda-detail-wrapper')
    ...
    $('body').addClass('lock');
    _this.$container
      .find('.agenda-feed-list-container')
